...
func (dr *dashboardRepo) GetDashboard(filter string) (int64, int64, int64, *[]de.FavouriteProducts, *[]de.MonthlyRevenue, error) {
	...
	// query := "SELECT CASE MONTH(transactions.created_at) WHEN 1 THEN 'January' WHEN 2 THEN 'February' WHEN 3 THEN 'March' WHEN 4 THEN 'April' WHEN 5 THEN 'May' WHEN 6 THEN 'June' WHEN 7 THEN 'July' WHEN 8 THEN 'August' WHEN 9 THEN 'September' WHEN 10 THEN 'October' WHEN 11 THEN 'November' WHEN 12 THEN 'December' END AS Month, SUM(transactions.total_price) AS Revenue FROM transactions WHERE YEAR(transactions.created_at) = YEAR(CURDATE()) AND transactions.canceled_reason IS NULL GROUP BY MONTH(transactions.created_at), transactions.created_at ORDER BY MONTH(transactions.created_at)"
	err = dr.db.Model(&te.Transaction{}).
	...
	return totalRevenue, totalOrder, totalUser, top3Order, monthlyRevenue, nil
}
